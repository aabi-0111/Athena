...
from app.core.exceptions import SHAPExplainerError

logger = logging.getLogger(__name__)

__all__ = ["SHAPExplainer", "SHAPExplainerError"]
...
class SHAPExplainer:
    ...
    def _validate_dataframe(self, X: pd.DataFrame) -> None:
        ...
        if X.columns.duplicated().any():
            ...
            raise SHAPExplainerError(f"Duplicate feature names detected: {duplicates}")

        if X.isnull().values.any():
            raise SHAPExplainerError("Input dataframe contains NaN values.")
    ...
    def summary_plot(
        ...
        *,
        ...
        try:
            ...
            raise SHAPExplainerError(f"Failed to save SHAP plot: {exc}") from exc
        finally:
            plt.close()
